...
def upload_sim_data(request):
    if request.method == 'POST' and request.FILES.get('json_file'):
        ...
        for genome, species_data in genome_species_data.items():
            species_names = species_data['species_names']
            counts_by_species = {species_name: [] for species_name in species_names}

            for snapshot in data['simulationCountData']['snapshots']:
                species_counts = {entry['speciesName']: entry.get('aliveCount', 0) for entry in snapshot['speciesDataList']}
                for species_name in species_names:
                    counts_by_species[species_name].append(species_counts.get(species_name, 0))

            combined_counts = [sum(count) for count in zip(*counts_by_species.values())]

            plt.plot(times, combined_counts, label='\n'.join(species_names))

        plt.xlabel('Czas')
        plt.ylabel('Żywe organizmy')
        plt.title('Zmiany populacji gatunków w czasie symulacji')
        plt.grid(True)
        ...
        return render(request, 'upload_sim_data.html', {'graphic': graphic, 'genome_species_data': genome_species_data})
